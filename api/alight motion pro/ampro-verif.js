...
/**
 * NAMA SCRAPE  :: AMPREM OOB VERIFIER
...
// Endpoint GET / (Misal dipanggil via /api/am/verify?email=target@mail.com&link=LINK_OOB)
router.get('/', async (req, res) => {
    try {
        const email = req.query.email;
        const link = req.query.linkoob;
        if (!email || !link) {
...
router.desc = "Memverifikasi link OOB untuk upgrade Alight Motion Pro. Parameter wajib: ?email=target@mail.com&link=OOB_LINK";
router.status = "ready";
router.type = "free";
