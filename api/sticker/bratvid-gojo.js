...
async function generateBratVideo(text) {
  ...
  const concatPath = path.join(os.tmpdir(), `concat-${timestamp}.txt`);
  const outputPath = path.join(os.tmpdir(), `bratvid-${timestamp}.mp4`);
  ...
  try {
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      const framePath = path.join(os.tmpdir(), `frame-${timestamp}-${i}.png`);
