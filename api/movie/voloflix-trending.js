...
async function getTrending(type = 'movie', country = 'ID', page = 1) {
  const url = new URL('https://veloflix.my.id/api/trending/country');
  ...
  return {
    ...
    items: list.map((item) => normalizeItem(item)),
  };
}
...
router.get('/', async (req, res) => {
  try {
    ...
    const country = (req.query.country || 'ID').toUpperCase();
    const page = parseInt(req.query.page) || 1;

    const data = await getTrending(type, country, page);
...
router.desc = "Mengambil daftar film atau serial TV yang sedang populer/trending berdasarkan negara.";
router.paramsConfig = {
  type: {
    type: "select",
    options: ["movie", "tv"],
