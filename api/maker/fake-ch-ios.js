...
// Daftar font lengkap dengan URL/Weight masing-masing
const FONTS = [
    {
...
async function getResources() {
    ...
    // Registrasi semua varian font ke GlobalFonts
    for (const f of FONTS) {
        try {
            const fontBuffer = await downloadBuffer(f.url);
            GlobalFonts.register(fontBuffer, f.family);
        } catch (err) {
            console.warn(`Gagal memuat font [${f.name}]:`, err.message);
        }
    }
