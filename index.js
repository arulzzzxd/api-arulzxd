...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>API-ARULZXD - REST CORE</title>
    ...
    #themeBg {
        ...
        background-image: 
            radial-gradient(circle at 50% 0%, rgba(0, 243, 255, 0.12) 0%, transparent 60%),
    ...
    /* Modern Cyber Glassmorphism Card */
    .glass-panel {
    ...
    .light-mode .glass-panel {
        ...
        box-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.05), inset 0 0 10px rgba(2, 132, 199, 0.02);
    }
    ...
    .cyber-card-glow::before {
        content: '';
        position: absolute;
        inset: -1px;
        border-radius: inherit;
        padding: 1px;
        background: linear-gradient(135deg, rgba(0, 243, 255, 0.4), transparent 60%, rgba(0, 243, 255, 0.2));
    ...
    .light-mode .cyber-card-glow::before {
        background: linear-gradient(135deg, rgba(2, 132, 199, 0.4), transparent 60%, rgba(2, 132, 199, 0.2));
    }
    ...
    /* Light Mode Form & Endpoint overrides */
    ...
    .neon-progress-bar {
        background: linear-gradient(90deg, #00b4d8, var(--neon-cyan));
    ...
    .cyber-popup-bg {
        ...
        background-image: radial-gradient(circle at 50% 20%, #03203c 0%, #010811 85%);
    }
...
<div id="cyber-loader-overlay">
    ...
    <div class="text-center px-4">
        <div id="loader-title-text" class="text-sm font-extrabold tracking-widest uppercase text-cyan-400 code-font mb-1 glow-cyan-text">
    ...
    <div class="w-64 sm:w-80 mt-6">
        <div class="flex items-center justify-between text-xs font-bold code-font mb-2">
            ...
            <span id="loader-percentage" class="text-cyan-400 text-sm glow-cyan-text">0%</span>
        </div>
        <div class="w-full h-2 bg-slate-900/90 rounded-full border border-cyan-500/40 overflow-hidden p-0.5">
...
<div id="welcomePopup" class="fixed inset-0 z-[99999] hidden">
  ...
  <div class="fixed inset-0 flex items-center justify-center p-4">
    <div class="glass-panel cyber-card-glow border border-cyan-500/40 rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md relative p-6 font-['Space_Grotesk'] text-slate-100 light-mode:text-slate-900 transition-all duration-300">
      
      <button id="closePopupBtn" class="absolute top-4 right-4 text-slate-400 hover:text-cyan-400 transition-colors bg-white/5 light-mode:bg-slate-200 hover:bg-white/10 rounded-full p-1.5 focus:outline-none border border-cyan-500/20 light-mode:border-slate-300">
      ...
      <div class="text-center mb-4">
        <h1 class="text-xl sm:text-2xl font-extrabold text-white light-mode:text-slate-900 leading-tight tracking-wide">
          Welcome to<br><span class="text-cyan-400 light-mode:text-cyan-600 glow-cyan-text">Arulz-XD API</span>
        </h1>
      </div>
      
      <div class="mb-4 rounded-xl overflow-hidden border border-cyan-500/30 light-mode:border-slate-300 bg-black/50 light-mode:bg-slate-100 relative group">
        <img src="https://arulz-xd.my.id/files/K4Sf61.png" alt="Welcome Banner" class="w-full h-auto object-cover max-h-48 transition-transform duration-500 group-hover:scale-105" />
      ...
      <div class="text-center text-slate-300 light-mode:text-slate-700 text-xs sm:text-sm mb-5 px-1 leading-relaxed">
        <p>Halo! 👋 Selamat datang di Arulz-XD API REST Core. API ini dirancang khusus untuk developer dengan kecepatan tinggi dan endpoint yang terus di-update. Silakan gunakan API Key di bawah ini untuk memulai.</p>
      </div>
      
      <div class="mb-5 flex justify-center">
        <div class="bg-slate-950/80 light-mode:bg-slate-200/60 rounded-full py-2 px-5 border-2 border-dashed border-cyan-500/50 light-mode:border-cyan-600/40 shadow-[0_0_15px_rgba(0,243,255,0.2)]">
          <span class="font-bold text-xs sm:text-sm text-slate-200 light-mode:text-slate-800 tracking-wide font-mono">
            apikey : <span id="welcomeApiKey" class="font-mono text-cyan-400 light-mode:text-cyan-700 select-all font-extrabold">${(req.user && req.user.apikey) ? req.user.apikey : 'Silakan Login'}</span>
          </span>
        </div>
      </div>
      
      <a href="/support" class="w-full bg-gradient-to-r from-cyan-600 via-cyan-500 to-cyan-400 hover:from-cyan-500 hover:to-cyan-300 text-slate-950 font-black py-3 px-6 rounded-xl shadow-lg shadow-cyan-500/30 transition-all text-sm block text-center tracking-wider uppercase cyber-glow-btn">
        Donate Sekarang
      </a>
    </div>
  </div>
...
<div id="profilePopup" class="fixed inset-0 z-[99999] hidden">
  ...
  <div class="fixed inset-0 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
    ...
    <div class="w-full max-w-[410px] cyber-popup-bg border-2 border-cyan-400 rounded-[32px] p-4 sm:p-5 shadow-[0_0_50px_rgba(0,243,255,0.45)] relative font-mono text-cyan-400 my-auto">
...
<div id="toast" class="fixed top-6 right-6 z-[9999] flex flex-col gap-3 pointer-events-none items-end"></div>
    ...
    <!-- Header Actions Trigger Button -->
    <div class="fixed top-6 right-6 z-40 flex items-center gap-3">
        <button id="bioMenuBtn" class="flex items-center justify-center w-11 h-11 rounded-xl glass-panel text-cyan-400 hover:text-white shadow-lg shadow-cyan-500/10 transition-all active:scale-95 focus:outline-none light-mode:text-slate-700 light-mode:hover:text-slate-900 border border-cyan-500/30">
    ...
    <!-- Sidebar Dropdown Nav Menu -->
    <div id="bioDropdown" class="fixed top-0 right-0 h-full w-72 bg-[#030914] border-l border-cyan-500/30 transform translate-x-full transition-transform duration-300 ease-in-out z-50 shadow-2xl flex flex-col p-6 font-['Space_Grotesk'] light-mode:bg-white light-mode:border-slate-200">
        <div class="flex items-center justify-between mb-5">
            <div class="flex gap-0 border border-cyan-500/30 rounded-lg p-0.5 bg-black/40">
            ...
            <div class="flex items-center gap-1.5">
                <button id="themeToggle" class="flex items-center justify-center w-9 h-9 rounded-lg transition-all active:scale-95 focus:outline-none border border-cyan-500/30 bg-slate-900/60 text-cyan-400 light-mode:bg-slate-100 light-mode:border-slate-300 light-mode:text-slate-900">
                ...
                <button id="closeMenuBtn" class="text-white hover:text-red-400 transition-colors p-2 border border-cyan-500/20 rounded-lg bg-slate-900/60 light-mode:text-slate-700 light-mode:bg-slate-100 light-mode:border-slate-300 light-mode:hover:text-red-500">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
        ...
        <div class="mb-3 flex flex-col gap-2">
            <a href="/login" class="flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-600 via-cyan-500 to-cyan-400 hover:from-cyan-500 hover:to-cyan-300 text-slate-950 font-black p-3 rounded-xl text-xs uppercase tracking-wider transition-all duration-200 shadow-lg shadow-cyan-500/20">
        ...
        <nav class="flex flex-col gap-1.5 text-xs font-semibold tracking-wider uppercase text-slate-300 flex-1 py-1 overflow-y-auto scrollbar-hide">
            <div class="text-[10px] font-bold text-slate-500 px-2 pt-2 pb-1 tracking-widest font-mono">PAGES</div>

            <a href="/" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
            ...
            <a href="/docs" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
                    <svg viewBox="0 0 24 24" fill="none" class="w-5 h-5 transform group-hover:scale-110 transition-transform duration-300">
                        <path d="M2 5c0-1.1.9-2 2-2h6.5l1.5 1.5L13.5 3H20c1.1 0 2 .9 2 2v13c0 1.1-.9 2-2 2h-6.5L12 18.5 10.5 20H4c-1.1 0-2-.9-2-2V5z" stroke="#00f3ff" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
            ...
            <a href="/store" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
            ...
            <a href="/changelog" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
                    <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="/uploader" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
                    <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="/pastecode" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
                    <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="/feedback" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
            ...
            <a href="/status" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
            ...
            <div class="text-[10px] font-bold text-slate-500 px-2 pt-3 pb-1 tracking-widest font-mono">LEGAL</div>

            <a href="/privacy" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
                    <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="/support" class="menu-link group flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cyan-950/40 transition-all duration-300 border border-transparent hover:border-cyan-500/30">
                <div class="w-8 h-8 rounded-lg bg-cyan-950/60 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.4)] transition-all shrink-0">
    ...
    <div id="menuOverlay" class="fixed inset-0 bg-black/70 backdrop-blur-xs hidden z-30 transition-opacity duration-300"></div>

    <!-- Main Container -->
    <div class="max-w-5xl mx-auto px-4 py-8 relative z-10">
        <header id="api" class="mb-10 text-center">
            <div class="flex items-center justify-center gap-3 mb-4">
                <span class="bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 light-mode:bg-cyan-100 light-mode:text-cyan-700 shadow-[0_0_15px_rgba(0,243,255,0.2)]">
                    <span class="w-2 h-2 rounded-full bg-cyan-400 animate-ping"></span> ONLINE // SYSTEM GATEWAY
                </span>
            </div>
            
            <div id="mainTitle" class="flex justify-center mb-3 min-h-[55px] items-center text-4xl md:text-5xl font-extrabold tracking-tight text-white">
                <img src="https://readme-typing-svg.demolab.com?font=Space+Grotesk&weight=800&size=30&pause=1000&color=00F3FF&center=true&vCenter=true&width=600&lines=Welcome+To+ArulzXD+API;Fast+%F0%9F%9A%80+Reliable+%E2%9A%A1;Free+REST+API+Services;Developer+Friendly+API" alt="Typing SVG" class="mx-auto" />
            </div>
            <p id="mainDescription" class="text-sm md:text-base font-medium tracking-wide text-slate-400 max-w-xl mx-auto leading-relaxed">
            ...
            <div class="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                <div class="glass-panel cyber-card-glow flex flex-col items-center justify-center p-4 rounded-2xl shadow-xl border border-cyan-500/20">
                    <div class="text-center font-['Space_Grotesk']">
                        <div id="liveClock" class="text-xl md:text-2xl font-black tracking-wider text-cyan-400 light-mode:text-cyan-600 font-mono glow-cyan-text">
                            00:00:00
                        </div>
                        <div id="liveDate" class="text-[9px] font-bold text-slate-400 tracking-wider mt-1 uppercase font-mono">
                            Loading...
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-panel cyber-card-glow flex flex-col items-center justify-center p-4 rounded-2xl shadow-xl border border-cyan-500/20">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 text-center font-mono">Limit Terpakai</span>
                    <div class="flex items-baseline gap-1 mt-0.5">
                        <span id="userLimitUsed" class="text-2xl font-black text-cyan-400 glow-cyan-text">0</span>
                        <span class="text-slate-500 font-bold text-xs">/</span>
                        <span id="userLimitMax" class="text-xs font-extrabold text-slate-400">100</span>
                    </div>
                    <span id="userLimitBadge" class="text-[8px] font-extrabold px-2 py-0.5 mt-1 rounded bg-slate-950 text-cyan-400 uppercase tracking-widest border border-cyan-500/30">FREE</span>
                </div>
                
                <div class="glass-panel cyber-card-glow flex flex-col items-center justify-center p-4 rounded-2xl shadow-xl border border-cyan-500/20">
                    <span id="stat-endpoints-title" class="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Total Endpoint</span>
                    <span id="totalEndpoints" class="text-2xl font-black text-cyan-400 mt-0.5 light-mode:text-cyan-600 glow-cyan-text">0</span>
                </div>
                
                <div class="glass-panel cyber-card-glow flex flex-col items-center justify-center p-4 rounded-2xl shadow-xl border border-cyan-500/20">
                    <span id="stat-categories-title" class="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Total Kategori</span>
                    <span id="totalCategories" class="text-2xl font-black text-cyan-400 mt-0.5 light-mode:text-cyan-600 glow-cyan-text">0</span>
                </div>
            </div>

            <!-- Domain & Feature Request Banner -->
            <div class="glass-panel cyber-card-glow max-w-4xl mx-auto mt-4 p-3.5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 border border-cyan-500/30">
                <div class="flex items-center gap-2.5 text-xs md:text-sm text-cyan-400 light-mode:text-cyan-700 font-mono">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    </svg>
                    <span class="underline break-all font-bold tracking-wide">https://arulz-xd.my.id</span>
                </div>
                <a href="/feedback" 
                   class="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-cyan-600 via-cyan-500 to-cyan-400 hover:from-cyan-500 hover:to-cyan-300 text-slate-950 font-black text-[11px] uppercase rounded-xl shadow-lg shadow-cyan-500/20 transition-all active:scale-95 light-mode:text-white text-center flex items-center justify-center gap-2 cyber-glow-btn">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
            ...
            <div class="flex justify-center gap-4 mt-4 max-w-4xl mx-auto">
                <a href="https://whatsapp.com/channel/0029VbAwdIyJJhzRMpjUcS3P" 
                   target="_blank" 
                   class="flex-1 glass-panel py-3 rounded-2xl text-xs font-bold uppercase tracking-wider hover:bg-cyan-500/10 transition-all text-center flex items-center justify-center gap-2 border border-cyan-500/20 text-slate-200 hover:border-cyan-400 hover:text-cyan-400">
            ...
            <!-- Music Player -->
            <div class="music-player-card glass-panel cyber-card-glow mt-6 max-w-2xl mx-auto rounded-2xl p-4 shadow-2xl relative overflow-hidden border border-cyan-500/30">
                ...
                <div class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div class="flex-1 min-w-0 text-left">
                            <h3 id="musicTitle" class="music-text-title text-white font-extrabold text-[13px] tracking-wide truncate m-0 uppercase">Loading...</h3>
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                ...
                                <div id="progressContainer" class="music-progress-bar-bg flex-1 h-1.5 bg-slate-900 rounded-full relative cursor-pointer overflow-hidden border border-white/5">
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        <button id="prevBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-slate-300 hover:text-cyan-400 transition-all active:scale-95 border border-cyan-500/20">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
                        </button>
                        <button id="playBtn" class="music-btn-nav w-10 h-10 flex items-center justify-center glass-panel rounded-lg text-slate-300 hover:text-cyan-400 transition-all active:scale-95 border border-cyan-500/30">
                            <svg id="playIcon" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                        </button>
                        <button id="nextBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-slate-300 hover:text-cyan-400 transition-all active:scale-95 border border-cyan-500/20">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M16 6h2v12h-2zm-10.5 12l8.5-6-8.5-6z"/></svg>
                        </button>
                        <button id="playlistToggleBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-slate-300 hover:text-cyan-400 transition-all active:scale-95 border border-cyan-500/20">
        ...
        <div class="mb-8">
            <div class="relative max-w-4xl mx-auto">
                <input 
                    ...
                    class="search-input w-full px-5 py-4 pl-12 text-xs rounded-2xl focus:outline-none focus:border-cyan-400 transition-all font-mono glass-panel border border-cyan-500/30 text-white placeholder-slate-400 light-mode:text-slate-900 light-mode:placeholder-slate-500 light-mode:focus:border-cyan-600 shadow-[0_0_20px_rgba(0,243,255,0.1)]"
                >
            ...
            <div id="categoryFilters" class="flex flex-wrap gap-2 mt-5 justify-start md:justify-center overflow-x-auto pb-2 scrollbar-hide max-w-4xl mx-auto"></div>
        </div>
        ...
        <footer id="siteFooter" class="mt-16 pt-6 border-t border-cyan-500/20 text-center text-[11px] text-slate-500 font-mono tracking-wider">
            © 2026 ARULZ-XD API REST CORE // ALL RIGHTS RESERVED
        </footer>
    </div>
    ...
    <div id="imageLightbox" class="fixed inset-0 bg-black/95 z-[100] hidden flex items-center justify-center p-4 opacity-0 transition-opacity duration-300 backdrop-blur-xs cursor-zoom-out">
        <div class="relative max-w-4xl max-h-[90vh] flex items-center justify-center">
            ...
            <button id="closeLightbox" class="absolute -top-12 right-0 text-white hover:text-cyan-400 transition-colors focus:outline-none flex items-center gap-1 bg-black/50 px-3 py-1.5 rounded-lg border border-cyan-500/30 text-xs font-mono">
                ✕ Close
            </button>
        </div>
    </div>
