...
<head>
    ...
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;600;700;800&family=Space+Grotesk:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    ...
    .glass-panel {
        ...
        box-shadow: 0 8px 32px 0 rgba(0, 243, 255, 0.08), inset 0 0 12px rgba(0, 243, 255, 0.05);
        position: relative;
    }
    .glass-panel::before {
        content: '';
    ...
    .light-mode #siteFooter { color: #64748b !important; border-color: rgba(0,0,0,0.08); }
    ...
    .light-mode .api-item input[type="text"]::placeholder { color: #64748b !important; }
    ...
    .ring-inner {
        ...
        inset: 22px;
        border-radius: 50%;
        border: 1.5px dotted var(--neon-cyan);
        opacity: 0.85;
    ...
    .cyber-popup-bg {
        ...
        background-image: radial-gradient(circle at 50% 30%, #032545 0%, #010a17 85%);
    }

    .double-border-cyan {
        background: #021124;
        border: 1.5px solid #00f3ff;
        box-shadow: 0 0 15px rgba(0, 243, 255, 0.25), inset 0 0 15px rgba(0, 243, 255, 0.05);
    }
    ...
    .cyber-pill-capsule {
        ...
        box-shadow: 0 0 10px rgba(0, 243, 255, 0.25), inset 0 0 8px rgba(0, 243, 255, 0.2);
    }
    ...
    .cyan-solid-header {
        background: linear-gradient(90deg, #00f3ff, #0284c7);
        color: #010811;
        font-weight: 900;
...
<div id="cyber-loader-overlay">
    ...
    <div class="text-center px-4">
        <div id="loader-title-text" class="text-sm font-extrabold tracking-widest uppercase text-cyan-400 code-font mb-1 neon-glow-cyan">
            Memuat Halaman<span class="animated-dots"></span>
        </div>
        <div class="text-[10px] text-cyan-300/70 font-mono tracking-widest uppercase">
    ...
    <div class="w-64 sm:w-80 mt-6">
        <div class="flex items-center justify-between text-xs font-bold code-font mb-2">
            <span class="text-slate-400 text-[10px] tracking-wider">SYSTEM STATUS</span>
            <span id="loader-percentage" class="text-cyan-400 text-sm font-mono font-extrabold">0%</span>
        </div>
        <div class="w-full h-2.5 bg-slate-950/90 rounded-full border border-cyan-500/40 overflow-hidden p-0.5 shadow-[0_0_10px_rgba(0,243,255,0.2)]">
            <div id="loader-progress-fill" class="h-full rounded-full neon-progress-bar w-0"></div>
        </div>
    </div>

    <div class="absolute bottom-6 text-[10px] font-mono text-cyan-400/50 uppercase tracking-widest">
        ARULZ-XD API v2.0 // CYBERPUNK EDITION
    </div>
</div>
...
<div id="welcomePopup" class="fixed inset-0 z-[99999] hidden">
  ...
  <div class="fixed inset-0 flex items-center justify-center p-4">
    <div class="glass-panel border-2 border-cyan-400/50 rounded-2xl shadow-[0_0_50px_rgba(0,243,255,0.3)] w-full max-w-sm sm:max-w-md relative p-6 font-['Space_Grotesk'] text-slate-100 light-mode:text-slate-900 transition-all duration-300">
      
      <button id="closePopupBtn" class="absolute top-4 right-4 text-cyan-400 hover:text-red-400 transition-colors bg-cyan-950/40 light-mode:bg-slate-200 hover:bg-red-950/50 rounded-full p-1.5 focus:outline-none border border-cyan-500/30">
      ...
      <div class="text-center mb-4">
        <h1 class="text-xl sm:text-2xl font-extrabold text-white light-mode:text-slate-900 leading-tight tracking-wide uppercase font-mono">
          Welcome to<br><span class="text-cyan-400 light-mode:text-cyan-600 neon-glow-cyan">Arulz-XD API</span>
        </h1>
      </div>
      
      <div class="mb-4 rounded-xl overflow-hidden border border-cyan-500/30 light-mode:border-slate-300 bg-black/60 light-mode:bg-slate-100 shadow-lg">
      ...
      <div class="mb-5 flex justify-center">
        <div class="cyber-pill-capsule py-2 px-5 text-center">
          <span class="font-bold text-xs sm:text-sm text-slate-200 light-mode:text-slate-800 tracking-wide font-mono">
            apikey : <span id="welcomeApiKey" class="text-cyan-400 light-mode:text-cyan-700 select-all font-extrabold">${(req.user && req.user.apikey) ? req.user.apikey : 'Silakan Login'}</span>
          </span>
        </div>
      </div>
      
      <a href="/support" class="w-full gold-metallic-button text-black font-extrabold py-3 px-6 rounded-xl shadow-lg transition-all text-sm block text-center tracking-wider uppercase active:scale-95">
        Donate Sekarang
      </a>
    </div>
  </div>
...
<div id="profilePopup" class="fixed inset-0 z-[99999] hidden">
  ...
  <div class="fixed inset-0 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
    ...
    <div class="w-full max-w-[420px] cyber-popup-bg border-2 border-cyan-400 rounded-[32px] p-5 sm:p-6 shadow-[0_0_50px_rgba(0,243,255,0.45)] relative font-mono text-cyan-400 my-auto">
        ...
        <div class="flex items-center justify-between mb-5 gap-2">
            <div class="relative w-20 h-20 sm:w-22 sm:h-22 flex-shrink-0">
                ...
                <div class="relative cursor-pointer w-full h-full" onclick="document.getElementById('avatarInput').click()">
                    <div class="w-full h-full rounded-full p-[3px] border-2 border-cyan-400 shadow-[0_0_20px_rgba(0,243,255,0.8)] overflow-hidden">
                        <img id="userAvatar" src="https://arulz-xd.my.id/files/X1F0Cn.png" class="w-full h-full rounded-full object-cover">
                    </div>
                    <div class="absolute bottom-0 right-0 bg-[#010811] text-cyan-400 p-1.5 rounded-full border border-cyan-400 shadow-[0_0_12px_rgba(0,243,255,0.9)]">
            ...
            <div class="flex-1 flex flex-col gap-2.5 min-w-0 px-2">
                <div class="cyber-pill-capsule py-1.5 px-3.5 text-center truncate">
                    <span id="userName" class="text-xs font-extrabold text-cyan-300 tracking-widest">loading...</span>
                </div>
                <div class="cyber-pill-capsule py-1.5 px-3.5 text-center truncate">
            ...
            <div id="planBoxContainer" class="relative w-20 h-28 flex flex-col items-center justify-center flex-shrink-0">
                <svg class="w-full h-full filter drop-shadow-[0_0_10px_rgba(0,243,255,0.7)]" viewBox="0 0 100 130" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <defs>
        ...
        <div class="double-border-cyan rounded-2xl p-3.5 mb-4 relative">
            <div class="flex items-center justify-between mb-2">
                <span class="text-[10px] font-extrabold text-cyan-300 border border-cyan-400 bg-[#010811] px-2.5 py-0.5 rounded-md uppercase tracking-wider">Api Key Kamu :</span>
            ...
            <div class="cyber-pill-capsule text-cyan-200 text-xs font-bold py-2 px-3 truncate mb-3 text-center tracking-wider">
            ...
            <button onclick="copyText(document.getElementById('userApiKey').innerText, 'API Key')" class="w-full gold-metallic-button text-xs py-2.5 rounded-xl uppercase tracking-widest active:scale-95 transition-all">
                SALIN API KEY
            </button>
        </div>

        <!-- Section 2: Limit User -->
        <div class="double-border-cyan rounded-2xl p-3.5 mb-4 text-center relative">
            ...
            <div class="py-0.5">
                <span class="inline-block cyber-pill-capsule text-cyan-300 px-6 py-1.5 text-sm font-extrabold tracking-widest">
        ...
        <div class="double-border-cyan rounded-2xl p-3.5 mb-4">
...
<div id="toast" class="fixed top-6 right-6 z-[9999] flex flex-col gap-3 pointer-events-none items-end"></div>
    ...
    <div class="fixed top-6 right-6 z-40 flex items-center gap-3">
        <button id="bioMenuBtn" class="flex items-center justify-center w-11 h-11 rounded-xl glass-panel text-cyan-400 hover:text-white shadow-[0_0_15px_rgba(0,243,255,0.2)] transition-all active:scale-95 focus:outline-none light-mode:text-slate-700 light-mode:hover:text-slate-900 border border-cyan-500/30">
    ...
    <!-- Sidebar Dropdown -->
    <div id="bioDropdown" class="fixed top-0 right-0 h-full w-72 bg-[#020a17] border-l border-cyan-500/30 transform translate-x-full transition-transform duration-300 ease-in-out z-50 shadow-[0_0_40px_rgba(0,243,255,0.2)] flex flex-col p-6 font-['Space_Grotesk'] light-mode:bg-white light-mode:border-slate-200">
        <div class="flex items-center justify-between mb-5">
            <div class="flex gap-0 border border-cyan-500/30 rounded-lg p-0.5 bg-black/50">
            ...
            <div class="flex items-center gap-1.5">
                <button id="themeToggle" class="flex items-center justify-center w-8 h-8 rounded-lg transition-all active:scale-95 focus:outline-none border border-cyan-500/30 bg-slate-900/60 text-cyan-400 light-mode:bg-slate-100 light-mode:border-slate-300 light-mode:text-slate-900">
                ...
                <button id="closeMenuBtn" class="text-cyan-400 hover:text-red-400 transition-colors p-1.5 border border-cyan-500/30 rounded bg-slate-900/40 light-mode:text-slate-700 light-mode:bg-slate-100 light-mode:border-slate-300 light-mode:hover:text-red-500">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
        ...
        <div class="mb-4 flex flex-col antialiased font-['Space_Grotesk']">
            <button onclick="openProfilePopup()" class="group relative flex items-center gap-3 bg-slate-950/90 text-white font-bold p-3 rounded-xl transition-all duration-300 text-xs tracking-wider uppercase overflow-hidden active:scale-95 border border-cyan-500/30 hover:border-cyan-400 shadow-[0_0_15px_rgba(0,243,255,0.15)] w-full">
                <div class="relative flex-shrink-0 z-10">
                    <img id="sidebarUserAvatar" src="${req.user.avatar}" class="w-8 h-8 rounded-full border border-cyan-400/50 object-cover shadow-sm">
        ...
        <div class="mb-3 flex flex-col gap-2">
            <a href="/login" class="flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white font-bold p-3 rounded-xl text-xs uppercase tracking-wider transition-all duration-200 shadow-[0_0_15px_rgba(0,243,255,0.3)]">
    ...
    <div class="text-[10px] font-bold text-cyan-400/70 px-2 pt-2 pb-1 tracking-widest font-mono">PAGES</div>

    <a href="/" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg viewBox="0 0 24 24" fill="none" class="w-5 h-5 transform group-hover:scale-110 transition-transform duration-300" style="filter: drop-shadow(0 0 3px rgba(0, 243, 255, 0.8));">
                <path d="M3 11l9-9 9 9v11H3V11z" stroke="#00f3ff" stroke-width="1.5" stroke-linejoin="round" />
                <path d="M19 8v-3h-2v1.5M10 16h4v6h-4v-6z" stroke="#00f3ff" stroke-width="1.5" stroke-linecap="round" />
                <path d="M7 13v6M9 13v6M7 16h2" stroke="#00f3ff" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M14 13v6l1.5-1.5 1.5 1.5v-6" stroke="#00f3ff" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M12 5v3M10 8h4" stroke="#00f3ff" stroke-width="0.5" stroke-linecap="round" />
                <circle cx="12" cy="5.5" r="0.5" fill="#00f3ff" />
                <circle cx="9" cy="9" r="0.4" fill="#00f3ff" />
                <circle cx="15" cy="9" r="0.4" fill="#00f3ff" />
            </svg>
        </div>
        <span class="font-medium text-cyan-100 group-hover:text-cyan-400 transition-colors duration-300">Dashboard</span>
    </a>

    <a href="/docs" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg viewBox="0 0 24 24" fill="none" class="w-5 h-5 transform group-hover:scale-110 transition-transform duration-300" style="filter: drop-shadow(0 0 3px rgba(0, 243, 255, 0.8));">
                <path d="M2 5c0-1.1.9-2 2-2h6.5l1.5 1.5L13.5 3H20c1.1 0 2 .9 2 2v13c0 1.1-.9 2-2 2h-6.5L12 18.5 10.5 20H4c-1.1 0-2-.9-2-2V5z" stroke="#00f3ff" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M3 6.5C3 5.7 3.7 5 4.5 5H11v13H4.5C3.7 18 3 17.3 3 16.5V6.5zM21 6.5c0-.8-.7-1.5-1.5-1.5H13v13h6.5c.8 0 1.5-.7 1.5-1.5V6.5z" stroke="#00f3ff" stroke-width="1.2" stroke-linejoin="round"/>
                <path d="M12 4v14.5" stroke="#00f3ff" stroke-width="1.2" stroke-linecap="round"/>
                <rect x="5.5" y="8" width="4.5" height="6.5" rx="0.8" stroke="#00f3ff" stroke-width="0.9" fill="none"/>
                <line x1="6.5" y1="10" x2="9" y2="10" stroke="#00f3ff" stroke-width="0.7" stroke-linecap="round"/>
                <line x1="6.5" y1="11.5" x2="9" y2="11.5" stroke="#00f3ff" stroke-width="0.7" stroke-linecap="round"/>
                <text x="16.8" y="11" fill="#00f3ff" font-size="2.6" font-weight="900" font-family="sans-serif" text-anchor="middle">DOCS</text>
            </svg>
        </div>
        <span class="font-medium text-cyan-100 group-hover:text-cyan-400 transition-colors duration-300">Docs</span>
    </a>

    <a href="/store" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
    ...
    <a href="/changelog" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <a href="/uploader" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <a href="/pastecode" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <a href="/feedback" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
    ...
    <a href="/status" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
    ...
    <div class="text-[10px] font-bold text-cyan-400/70 px-2 pt-3 pb-1 tracking-widest font-mono">LEGAL</div>

    <a href="/privacy" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
            <svg class="w-4 h-4 text-cyan-400 transform group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <a href="/support" class="menu-link group flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-cyan-950/40 transition-all duration-300">
        <div class="w-8 h-8 rounded-lg bg-cyan-950/40 border border-cyan-500/30 flex items-center justify-center group-hover:border-cyan-400/60 group-hover:shadow-[0_0_12px_rgba(0,243,255,0.3)] transition-all shrink-0">
    ...
    <div id="menuOverlay" class="fixed inset-0 bg-black/70 backdrop-blur-sm hidden z-30 transition-opacity duration-300"></div>

    <div class="max-w-5xl mx-auto px-4 py-8 relative z-10">
        <header id="api" class="mb-10 text-center">
            <div class="flex items-center justify-center gap-3 mb-3">
                <span class="bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 px-4 py-1.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest flex items-center gap-2 shadow-[0_0_12px_rgba(0,243,255,0.25)] light-mode:bg-cyan-100 light-mode:text-cyan-700 font-mono">
                    <span class="w-2 h-2 rounded-full bg-cyan-400 animate-ping"></span> ONLINE SYSTEM
                </span>
            </div>
            ...
            <p id="mainDescription" class="text-sm md:text-base font-normal tracking-wide text-slate-300 max-w-xl mx-auto leading-relaxed">
            ...
            <div class="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
                <div class="glass-panel flex flex-col items-center justify-center p-4 rounded-2xl shadow-lg border border-cyan-500/30">
                    <div class="text-center font-['Space_Grotesk']">
                        <div id="liveClock" class="text-xl md:text-2xl font-extrabold tracking-wider text-cyan-400 light-mode:text-cyan-600 font-mono neon-glow-cyan">
                            00:00:00
                        </div>
                        <div id="liveDate" class="text-[9px] font-bold opacity-75 tracking-wider mt-1 uppercase font-mono text-cyan-300">
                            Loading...
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-panel flex flex-col items-center justify-center p-4 rounded-2xl shadow-lg border border-cyan-500/30">
                    <span class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 text-center font-mono">Limit Terpakai</span>
                    <div class="flex items-baseline gap-0.5 mt-0.5 font-mono">
                        <span id="userLimitUsed" class="text-2xl font-black text-cyan-400 neon-glow-cyan">0</span>
                        <span class="text-slate-500 font-bold text-xs">/</span>
                        <span id="userLimitMax" class="text-xs font-bold text-slate-400">100</span>
                    </div>
                    <span id="userLimitBadge" class="text-[8px] font-extrabold px-2 py-0.5 mt-1 rounded bg-slate-900 text-cyan-400 uppercase tracking-widest border border-cyan-500/30 font-mono">FREE</span>
                </div>
                
                <div class="glass-panel flex flex-col items-center justify-center p-4 rounded-2xl shadow-lg border border-cyan-500/30">
                    <span id="stat-endpoints-title" class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 font-mono">Total Endpoint</span>
                    <span id="totalEndpoints" class="text-2xl font-black text-cyan-400 mt-0.5 light-mode:text-cyan-600 font-mono neon-glow-cyan">0</span>
                </div>
                
                <div class="glass-panel flex flex-col items-center justify-center p-4 rounded-2xl shadow-lg border border-cyan-500/30">
                    <span id="stat-categories-title" class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 font-mono">Total Kategori</span>
                    <span id="totalCategories" class="text-2xl font-black text-cyan-400 mt-0.5 light-mode:text-cyan-600 font-mono neon-glow-cyan">0</span>
                </div>
            </div>

            <div class="glass-panel max-w-4xl mx-auto mt-4 p-3.5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 border border-cyan-500/30">
                <div class="flex items-center gap-2 text-xs md:text-sm text-cyan-400 light-mode:text-cyan-700 font-mono">
                    ...
                    <span class="underline break-all font-semibold tracking-wide">https://arulz-xd.my.id</span>
                </div>
                <a href="/feedback" 
                   class="w-full sm:w-auto px-5 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-black text-[11px] uppercase rounded-xl shadow-[0_0_15px_rgba(0,243,255,0.4)] transition-all active:scale-95 light-mode:text-white text-center flex items-center justify-center gap-1.5 tracking-wider font-mono">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
            ...
            <div class="flex justify-center gap-4 mt-4 max-w-4xl mx-auto">
                <a href="https://whatsapp.com/channel/0029VbAwdIyJJhzRMpjUcS3P" 
                   target="_blank" 
                   class="flex-1 glass-panel py-3 rounded-2xl text-xs font-bold uppercase tracking-wider hover:bg-cyan-500/10 transition-all text-center flex items-center justify-center gap-2 border border-cyan-500/30 text-slate-200 hover:text-cyan-400 font-mono">
            ...
            <div class="music-player-card glass-panel mt-6 max-w-2xl mx-auto rounded-2xl p-4 shadow-xl relative overflow-hidden border border-cyan-500/30">
                <audio id="audioElement"></audio>
                <div class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                        <div class="relative w-14 h-14 rounded-xl overflow-hidden bg-black/60 flex-shrink-0 border border-cyan-500/40 shadow-md">
                            <img id="musicCoverImg" src="" alt="Cover" class="w-full h-full object-cover">
                        </div>
                        <div class="flex-1 min-w-0 text-left">
                            <h3 id="musicTitle" class="music-text-title text-white font-bold text-[13px] tracking-wide truncate m-0 uppercase font-mono">Loading...</h3>
                            <p id="musicArtist" class="music-text-artist text-slate-400 text-[11px] font-medium truncate mt-0.5">-</p>
                            <div class="flex items-center gap-2 mt-2">
                                <span id="currentTime" class="text-[9px] text-cyan-400 font-mono w-7 text-left">0:00</span>
                                <div id="progressContainer" class="music-progress-bar-bg flex-1 h-1 bg-cyan-950/60 rounded-full relative cursor-pointer border border-cyan-500/20">
                                    <div id="progressBar" class="h-full bg-cyan-400 rounded-full w-0 transition-all duration-300 shadow-[0_0_8px_rgba(0,243,255,0.8)]"></div>
                                </div>
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        <button id="prevBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-cyan-400 hover:text-white transition-all active:scale-95 border border-cyan-500/30">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
                        </button>
                        <button id="playBtn" class="music-btn-nav w-10 h-10 flex items-center justify-center glass-panel rounded-lg text-cyan-400 hover:text-white transition-all active:scale-95 border border-cyan-500/30 shadow-[0_0_12px_rgba(0,243,255,0.3)]">
                            <svg id="playIcon" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                        </button>
                        <button id="nextBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-cyan-400 hover:text-white transition-all active:scale-95 border border-cyan-500/30">
                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M16 6h2v12h-2zm-10.5 12l8.5-6-8.5-6z"/></svg>
                        </button>
                        <button id="playlistToggleBtn" class="music-btn-nav w-8 h-8 flex items-center justify-center glass-panel rounded-lg text-cyan-400 hover:text-white transition-all active:scale-95 border border-cyan-500/30">
                ...
                <div id="playlistPanel" class="music-playlist-border hidden mt-4 pt-4 border-t border-cyan-500/20 max-h-40 overflow-y-auto space-y-1 light-mode:border-slate-200"></div>
            </div>
        ...
        <div class="mb-8">
            <div class="relative max-w-4xl mx-auto">
                <input 
                    ...
                    class="search-input w-full px-5 py-4 pl-12 text-xs rounded-2xl focus:outline-none focus:border-cyan-400 focus:shadow-[0_0_20px_rgba(0,243,255,0.35)] transition-all font-mono glass-panel border border-cyan-500/30 text-white placeholder-slate-400 light-mode:text-slate-900 light-mode:placeholder-slate-500 light-mode:focus:border-cyan-600"
                >
                <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-cyan-400" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
            </div>
            <div id="categoryFilters" class="flex flex-wrap gap-2.5 mt-4 justify-start md:justify-center overflow-x-auto pb-2 scrollbar-hide max-w-4xl mx-auto"></div>
        </div>

        <div id="noResults" class="text-center py-12 hidden">
            <div class="flex justify-center mb-3">
                <svg class="w-12 h-12 text-amber-400 animate-pulse" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
            </div>
            <h3 id="no-results-title" class="text-sm font-bold mb-1 text-white font-mono uppercase tracking-wider">Endpoint tidak ditemukan</h3>
        ...
        <footer id="siteFooter" class="mt-16 pt-6 border-t border-cyan-500/20 text-center text-[11px] text-slate-500 font-mono tracking-widest uppercase">
            © Arulz-XD REST API
        </footer>
    </div>

    <div id="imageLightbox" class="fixed inset-0 bg-black/95 z-[100] hidden flex items-center justify-center p-4 opacity-0 transition-opacity duration-300 backdrop-blur-md cursor-zoom-out">
        <div class="relative max-w-4xl max-h-[90vh] flex items-center justify-center">
            <img id="lightboxImage" src="" alt="Preview" class="max-w-full max-h-[85vh] rounded-xl shadow-[0_0_30px_rgba(0,243,255,0.4)] object-contain scale-95 transition-transform duration-300 border border-cyan-500/30" />
            <button id="closeLightbox" class="absolute -top-12 right-0 text-cyan-400 hover:text-white transition-colors focus:outline-none flex items-center gap-1 bg-black/60 px-3 py-1.5 rounded-lg border border-cyan-500/30 text-xs font-mono">
                ✕ Close
            </button>
        </div>
    </div>
